...
public class CustomAuthenticationFailure implements ApplicationListener<AuthenticationFailureBadCredentialsEvent> {
	...
	public void onApplicationEvent(AuthenticationFailureBadCredentialsEvent event) {
		...
		if (sessionDetails.getUsername() == null) {
			...
			sessionDetails.setUserDownAttempts(1);
		} else if (!sessionDetails.getUsername().equals(user.toString())) {
			sessionDetails.setUsername(user.toString());
			sessionDetails.setUserDownAttempts(1);
		} else
			sessionDetails.setUserDownAttempts(sessionDetails.getUserDownAttempts() + 1);

		System.out.println(" In authentication handler"+" Username  : "+ sessionDetails.getUsername()+" Failed attempts : " + sessionDetails.getUserDownAttempts());
